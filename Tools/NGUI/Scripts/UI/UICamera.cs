...
public class UICamera : MonoBehaviour
{
    ...
    bool hasUiEventProcessed = false;
		for (int i = 0; i < Input.touchCount; ++i)
		{
			Touch touch = Input.GetTouch(i);
			currentTouchID = allowMultiTouch ? touch.fingerId : 1;
			bool canContinue = false;
			if(mTouches.Count>0&&!mTouches.ContainsKey(currentTouchID))
			{
				foreach(MouseOrTouch mot in mTouches.Values)
				{
					if(mot!=null && mot.pressed!=null)
					{
						canContinue = true;
						break;
					}
      ...
      hasUiEventProcessed = Raycast(currentTouch.pos, out lastHit);
      if (!hasUiEventProcessed) hoveredObject = fallThrough;       
      ...
      //NGUI鐨刟llowMultiTouch涓哄師濮嬮夐」锛屼絾鏄?叾鍙栫殑纭?疄Unity鐨処nput,
      //杩欐牱鍦ㄦ憞鏉呹鎸夐挳鐨勬搷浣滄椂锛屼細瀵艰嚧NGUI鎸夐挳涓嶅ソ浣跨殑鎯呭喌鍙戠敓
			if (!allowMultiTouch &&hasUiEventProcessed) break;
		}
