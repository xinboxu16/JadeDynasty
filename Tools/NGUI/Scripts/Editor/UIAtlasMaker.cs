...
public class UIAtlasMaker : EditorWindow
{
	...
	void OnGUI ()
	{
		...
		if (NGUISettings.atlas != null)
		{
            ...
            else if (GUILayout.Button("Export Sprites"))
            {
                if (NGUISettings.atlas == null)
                {
                    Debug.Log("No Atlas selected.");
                }
                else
                {
                    UIAtlas atlas = NGUISettings.atlas;
                    List<UISpriteData> atlaDatas = atlas.GetSprites();
                    string path = EditorUtility.OpenFolderPanel("Open...", "", "");
                    Debug.Log("Export Sprites textures Path" + path);
                    EditorUtility.DisplayProgressBar("Export Png File", "Show progress 0/" + atlaDatas.Count.ToString(), 0f);
                    for (int i = 0; i < atlaDatas.Count; i++)
                    {
                        UISpriteData spriteData = atlaDatas[i];
                        UIAtlasMaker.SpriteEntry se = UIAtlasMaker.ExtractSprite(atlas, spriteData.name);
                        byte[] bytes = se.tex.EncodeToPNG();
                        File.WriteAllBytes(path + "/" + spriteData.name + ".png", bytes);
                        Debug.Log("Export Sprites textures" + spriteData.name);
                        if (se.temporaryTexture) DestroyImmediate(se.tex);
                        EditorUtility.DisplayProgressBar("Export Png File", "Show progress  " + (i+1).ToString() + "/" + atlaDatas.Count.ToString(), (i+1) * 1.0f / atlaDatas.Count);
                    }
                    //int i = 0;
                    //foreach(UISpriteData spriteData in atlaDatas)
                    //{
                    //    UIAtlasMaker.SpriteEntry se = UIAtlasMaker.ExtractSprite(atlas, spriteData.name);
                    //    byte[] bytes = se.tex.EncodeToPNG();
                    //    File.WriteAllBytes(path + "/" + spriteData.name + ".png", bytes);
                    //    Debug.Log("Export Sprites textures" + spriteData.name);
                    //    if (se.temporaryTexture) DestroyImmediate(se.tex);
                    //    EditorUtility.DisplayProgressBar("Export Png File", "Show progress  " + i.ToString() + "/" + atlaDatas.Count.ToString(), (float)(i / atlaDatas.Count));
                    //    i++;
                    //}
                    EditorUtility.ClearProgressBar();
                }
